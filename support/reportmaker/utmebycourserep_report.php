<?php
	...
	if  (isset($_POST["action"]))
	{
		...
		if (isset($_POST["course"])) $course=trim($_POST["course"]);
		...
		{			
			$sql = "SELECT * FROM candidates WHERE (TRIM(session)='".$session."') ";
		
			if (trim($faculty) != '') $sql .= " AND (TRIM(facabrev)='".$faculty."')";
			if (trim($dept) != '') $sql .= " AND (TRIM(dept)='".$dept."')";
			if (trim($course) != '') $sql .= " AND (TRIM(courseabrev)='".$course."')";
...
<thead>
 <tr bgcolor="#EEEEEE">
    ...
    <th align="center" valign="middle" width="20%"><b><font size="11pt" face="Arial">Candidate Name</font></b></th>
    ...
    <th align="center" valign="middle" width="15%"><b><font size="11pt" face="Arial">Course Abrev.</font></b></th>
...
<tr align="center">
	...
	<td valign="middle" align="center" width="20%"><font size="10pt" face="Georgia, Times New Roman, Times, serif">'.ucwords(strtolower($cname)).'</font></td>										
	...
	<td valign="middle" align="center" width="15%"><font size="10pt" face="Georgia, Times New Roman, Times, serif">'.ucwords(strtolower($cou)).'</font></td>									
	...
	<td valign="middle" align="center" width="8%"><font size="10pt" face="Georgia, Times New Roman, Times, serif">'.$rem.'</font></td>
				...
				#Get Course Fullname
				$sql = "SELECT course FROM courses WHERE (TRIM(facabrev)='".$faculty."') AND (TRIM(dept)='".$dept."') AND (TRIM(courseabrev)='".$course."')";
				#$file = fopen('idong.txt',"w"); fwrite($file,$sql); fclose($file);
			
				$res = mysql_query($sql, $conn); 
				$rw = mysql_fetch_array($res);
				
				if ($rw['course']) $coursename=trim($rw['course']); else $coursename=trim($course);
				
				$reportheader .= '
					<div><p align="center" >
					<font size="13pt" face="Arial,Helvetica, sans-serif"><b>'.$session.' POST UTME RESULT</b></font>
					</p>
					<p align="center">
					<font size="13pt" face="Arial,Helvetica, sans-serif"><b>COURSE: '.strtoupper($coursename).'</b></font>
					</p></div>';
				...
				if ($logo)
				{
					if (file_exists('../images/'.$logo))
					{
						if (file_exists("../images/".$logo))
						{
							...
							{
								$pdf->SetHeaderData('', 0, '', 'POST UTME Result By Course Of Study');
							}else
							{
								$pdf->SetHeaderData('', 0, '', 'POST UTME Result By Course Of Study');	
							}
				...
				$outputfile="../reports/CoursePostUTMEResultReport.pdf";
				
								
				$pdf->Output($outputfile, 'F');
					
				$rows[]="CoursePostUTMEResultReport.pdf";
