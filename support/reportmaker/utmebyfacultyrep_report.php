...
<tr align="center">
	...
	<td valign="middle" align="center" width="8%"><font size="10pt" face="Georgia, Times New Roman, Times, serif">'.$rem.'</font></td>
				...
				#Get Faculty Fullname
				$sql = "SELECT faculty FROM faculties WHERE (TRIM(facabrev)='".$faculty."')";
				#$file = fopen('idong.txt',"w"); fwrite($file,$sql); fclose($file);
			
				$res = mysql_query($sql, $conn); 
				$rw = mysql_fetch_array($res);
				
				if ($rw['faculty']) $facultyname=trim($rw['faculty']); else $facultyname=trim($faculty);
				...
				$reportheader .= '
					...
					<p align="center">
					<font size="13pt" face="Arial,Helvetica, sans-serif"><b>FACULTY: '.strtoupper($facultyname).'</b></font>
					</p></div>';
				...
				if ($logo)
				{
					if (file_exists('../images/'.$logo))
					{
						if (file_exists("../images/".$logo))
						{
							...
							{
								$pdf->SetHeaderData('', 0, '', 'POST UTME Result By Faculty');
							}else
							{
								$pdf->SetHeaderData('', 0, '', 'POST UTME Result By Faculty');	
							}
				...
				$outputfile="../reports/FacultyPostUTMEResultReport.pdf";
				
								
				$pdf->Output($outputfile, 'F');
					
				$rows[]="FacultyPostUTMEResultReport.pdf";
